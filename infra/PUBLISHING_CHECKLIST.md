...
- Nodo del tipo correspondiente (`MusicGroup`/`Organization`, `Person`, `Place`,
  `MusicAlbum`, `MusicComposition`…) con `@id` canónico para el knowledge graph.
- `VideoObject` (con `about` → la composición) si la página embebe vídeo.
- Validar en Rich Results Test tras desplegar.
