...
export const metadata: Metadata = {
  title: "Baja de la newsletter · Entre Interiores",
  description: "Baja de la newsletter de Entre Interiores.",
...
export default async function BajaPage({
  ...
  if (!token) {
    result = { status: "invalid_token", message: "Falta el token de baja." };
  } else {
    try {
      result = await apiFetch<StatusResponse>(
        `/public/newsletter/unsubscribe?token=${encodeURIComponent(token)}`,
        { authenticated: false },
      );
    } catch (e) {
      result = {
  ...
  const isOk =
    result.status === "unsubscribed" || result.status === "already_unsubscribed";
      ...
      <main className="px-5 md:px-14 py-14 md:py-20 max-w-[680px] mx-auto text-center">
        <Breadcrumbs
          ...
          items={[
            ...
            { label: "Baja", href: "/newsletter/baja" },
          ]}
        />
        ...
        <h1 className="font-serif text-4xl md:text-[56px] text-ink leading-[0.98] tracking-[-1px] mb-6">
          {isOk ? "Te hemos dado de baja" : "Algo no cuadra"}
        </h1>
