...
// Bloque "En el diario": hasta 3 posts del blog más relevantes para esta
// página (precálculo semanal híbrido: menciones explícitas + semántica).
// Si aún no hay precálculo, cae a las menciones explícitas en tiempo real.

export default async function RelatedPosts({
  entityType,
  slug,
  heading = "En el diario",
}: {
  entityType: string;
  slug: string;
  heading?: string;
  ...
  try {
    items = await apiFetch<PublicPostListItem[]>(
      `/public/posts/related?entity_type=${encodeURIComponent(entityType)}&slug=${encodeURIComponent(slug)}`,
      { authenticated: false },
    );
    if (items.length === 0) {
      // Fallback: menciones explícitas en tiempo real (mientras no haya
      // precálculo, p.ej. tras publicar antes del cron semanal).
      items = await apiFetch<PublicPostListItem[]>(
        `/public/posts/mentioning?slug=${encodeURIComponent(slug)}&limit=3`,
        { authenticated: false },
      );
    }
  } catch {
  ...
  return (
    <section className="mt-16 border-t border-divider pt-10">
      ...
      <ul className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 gap-x-6 gap-y-8">
        {items.map((p) => (
          <li key={p.slug}>
            <Link href={`/blog/${p.slug}`} data-cursor="hover" className="group block">
